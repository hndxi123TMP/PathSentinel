...
public class AndroidCallGraphPatching extends SceneTransformer {
    ...
    public AndroidCallGraphPatching(ManifestAnalysis manifestAnalysis) {
        ...
        // Add new comprehensive Android component patchers
        _patchers.add(new BroadcastReceiverPatcher(_fakeClass, _manifestAnalysis));
        _patchers.add(new ContentProviderPatcher(_fakeClass, _manifestAnalysis));
        _patchers.add(new MessengerPatcher(_fakeClass, _manifestAnalysis));
        
        System.err.println("PATHSENT-PATCHING: Registered " + _patchers.size() + " call graph patchers:");
        for (CallGraphPatcher patcher : _patchers) {
            System.err.println("  - " + patcher.getClass().getSimpleName());
        }
    }
    ...
    private SootClass createFakePatchClass() {
        SootClass patchClass = new SootClass(
                "pathsent.patching.FakeAndroidCallGraphPatching", Modifier.PUBLIC);
